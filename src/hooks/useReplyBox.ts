...
export type SetSelectionState = (entityId: StoryItem["id"], selected: boolean, type: ReplyBoxType) => void|Promise<{ isSuccess: boolean }>;

export type GetSelectionState = (entityId: StoryItem["id"], type: ReplyBoxType) => void|Promise<Array<GetStateResponse<string>>>;

export type DeleteSelectionState = (entityId: StoryItem["id"], type: ReplyBoxType) => void|Promise<boolean>;
...
export const registerReplyBoxNotesAdditionsTargetAction = (client: IDeskproClient, state: State) => {
    ...
    const entities: Array<StoryItem> = get(state, ["linkedStoriesResults", "list"], []);
    ...
    Promise
        .all(entities.map((entity: StoryItem) => client.getState<{ selected: boolean }>(noteKey(ticketId, entity.id))))
        .then((flags) => {
            client.registerTargetAction("shortcutReplyBoxNoteAdditions", "reply_box_note_item_selection", {
...
export const registerReplyBoxEmailsAdditionsTargetAction = (client: IDeskproClient, state: State): void => {
    const ticketId = state?.context?.data.ticket.id;
    const entities: Array<StoryItem> = get(state, ["linkedStoriesResults", "list"], []);
    ...
    Promise
        .all(entities.map((entity: StoryItem) => {
            return client.getState<{ selected: boolean }>(emailKey(ticketId, entity.id))
        }))
        .then((flags) => {
            return client.registerTargetAction("shortcutReplyBoxEmailAdditions", "reply_box_email_item_selection", {
                title: `Add to Shortcut`,
                payload: entities.map((entity, idx) => ({
...
const useReplyBox = (): ReturnUseReplyBox => {
    ...
    const deleteSelectionState = useCallback((entityId, type) => {
        if (!ticketId) { return }
    ...
    const debounceTargetAction = useDebouncedCallback<(a: TargetAction) => void>(
        (action: TargetAction) => match<string>(action.name)
            .with("shortcutOnReplyBoxEmail", () => {
                ...
                client.getState<{ id: string; selected: boolean }>(`tickets/${ticketId}/shortcut/emails/selection/*`)
                    .then((r) => {
                        const storyIds = r
                        ...
                        return Promise.all(
                            storyIds.map((storyId) => createStoryComment(client, storyId, email))
                        );
                    })
            ...
            .with("shortcutOnReplyBoxNote", () => {
                ...
                client.setBlocking(true);
                client.getState<{ id: string; selected: boolean }>(`tickets/${ticketId}/shortcut/notes/selection/*`)
                    .then((r) => {
                        const storyIds = r
                            .filter(({ data }) => data?.selected)
                            .map((({ data }) => data?.id as string));

                        return Promise.all(storyIds.map((storyId) => createStoryComment(client, storyId, note)));
                    })
                    .then((comments: Comment[]) => {
                        const stories = state.linkedStoriesResults?.list ?? [];
            ...
            .with("shortcutReplyBoxEmailAdditions", () => {
                (action.payload ?? []).forEach((selection: { id: string; selected: boolean; }) => {
                    ...
                    if (state.context?.data.ticket.id) {
                        client?.setState(
                            emailKey(ticketId, selection.id),
